...
function useMyLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      const place = { id: 'my_location', name: state.language === 'en' ? 'My Location' : 'Lokasi Saya', lat: pos.coords.latitude, lng: pos.coords.longitude };
...
function renderRouteInfo() {
  ...
  const steps = (p.legs?.[0]?.steps || []).slice(0, 5).map((step) => {
    ...
    const translated = state.language === 'id' ? translateInstructionToId(original) : original;
...
function translateInstructionToId(text) {
  let out = String(text || '');
  const replacements = [
    [/^Drive southwest/i, 'Jalan ke arah barat daya'],
    [/^Drive southeast/i, 'Jalan ke arah tenggara'],
    [/^Drive northwest/i, 'Jalan ke arah barat laut'],
    [/^Drive northeast/i, 'Jalan ke arah timur laut'],
    [/^Drive north/i, 'Jalan ke arah utara'],
    [/^Drive south/i, 'Jalan ke arah selatan'],
    [/^Drive west/i, 'Jalan ke arah barat'],
    [/^Drive east/i, 'Jalan ke arah timur'],
    [/^Head southwest/i, 'Mulai ke arah barat daya'],
    [/^Head southeast/i, 'Mulai ke arah tenggara'],
    [/^Head northwest/i, 'Mulai ke arah barat laut'],
    [/^Head northeast/i, 'Mulai ke arah timur laut'],
    [/^Head north/i, 'Mulai ke arah utara'],
    [/^Head south/i, 'Mulai ke arah selatan'],
    [/^Head west/i, 'Mulai ke arah barat'],
    [/^Head east/i, 'Mulai ke arah timur'],
    [/^Turn left onto /i, 'Belok kiri ke '],
    [/^Turn right onto /i, 'Belok kanan ke '],
    [/^Bear left onto /i, 'Belok agak kiri ke '],
    [/^Bear right onto /i, 'Belok agak kanan ke '],
    [/^Slight left onto /i, 'Belok sedikit kiri ke '],
    [/^Slight right onto /i, 'Belok sedikit kanan ke '],
    [/^Make a U-turn to stay on /i, 'Putar balik untuk tetap di '],
    [/^Continue straight onto /i, 'Lanjut lurus ke '],
    [/^Continue on /i, 'Lanjut di '],
    [/^Keep left to stay on /i, 'Ambil kiri untuk tetap di '],
    [/^Keep right to stay on /i, 'Ambil kanan untuk tetap di '],
    [/^At the roundabout, take the /i, 'Di bundaran, ambil '],
    [/^At the roundabout, take /i, 'Di bundaran, ambil '],
    [/^Exit the roundabout onto /i, 'Keluar bundaran ke '],
    [/^You have arrived at your destination/i, 'Anda telah tiba di tujuan'],
    [/destination is on the left/i, 'tujuan ada di kiri'],
    [/destination is on the right/i, 'tujuan ada di kanan'],
    [/ toward /gi, ' menuju ']
  ];
